...
const Myvehicle = () => {
  ...
  const VechileData = async () => {
    await axios.get(`${app}api/VehicleTypes/`).then((response) => {
  ...
  const vechileDataGet = async () => {
    await axios.get(`${app}AddvehicleGet/${id}`).then((response) => {
      const Data = response.data;
  ...
  const getbyid = async (items) => {
    ...
    await axios
      .get(`${app}AddvehicleGetImages/${items.vehicle_id}`)
      .then((response) => {
        const Data = response.data;
    ...
    await axios
      .get(`${app}AddVehicleGetByID/${items.vehicle_id}`)
      .then((response) => {
  ...
  const handleEditSubmit = async () => {
    ...
    if (ImageData.AdditionalImages) {
      ImageData.AdditionalImages.forEach((img, index) => {
        if (img) {
          // Only append if the image is not null
          formData.append("AdditionalImages", img);
          formData.append(
            "AdditionalImagesID",
            ImageData.AdditionalImageID[index]
          );
        }
      });
    }
    ...
    <>
      ...
      <div className=" ">
        <div className="flex justify-between items-start pt-3 ml-8 mr-8">
          <span className="font-semibold text-lg md:text-2xl tracking-[1px]">
            My Vehicles
          </span>
          <button
            onClick={() => toadd()}
        ...
        <div className="mt-6 px-4 md:px-8">
          {Getdata.map((items, i) => (
            <div
              key={i}
              className="bg-white mb-4 rounded-2xl shadow-[0px_4px_11px_8px_rgba(0,0,0,0.1)] overflow-hidden"
            >
              <div className="p-4 md:p-6 flex flex-col md:flex-row gap-6">
                <div className="w-full md:w-[200px]">
                  <img
                    src={`${app}uploads/${items.Img}`}
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div
                            ...
                            className={`w-12 h-6 rounded-full flex items-center cursor-pointer transition-all duration-300 ${
                              selectedStates[i] ? "bg-green-500" : "bg-gray-400"
                            }`}
                          >
                            <div
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded-md text-sm transition-colors duration-300"
                          >
                            Edit
                          </button>
              ...
              <form className="space-y-6">
                ...
                <div className="">
                  ...
                  <div className="grid grid-cols-2  items-center">
                    <label
                      htmlFor="front-image"
                      className="w-100 p-6 border-2 border-dashed border-gray-300 rounded-md flex flex-col items-center justify-center bg-white shadow-sm hover:bg-gray-50 cursor-pointer transition"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                      ...
                      <div key={id} className="relative">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const AdditionalImageID = [
                              ...ImageData.AdditionalImageID,
                            ];
                ...
                <div className="flex  gap-4 mt-8 justify-center">
                  <span
                    ...
                    className="bg-indigo-600 hover:cursor-pointer text-white  text-lg px-6 py-3 rounded-md font-semibold hover:bg-indigo-700 transition"
                  >
                    Update
                  </span>
                </div>
